...
3. Back on Unity, go to `Window` > `Package Manager`, then in the new window `+` > `Add package from disk...`
...
You should now see a `Sex Script` menu item in the top menu bar and right clicking on asset folder should give you "HFramework" options.
