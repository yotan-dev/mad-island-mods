...
namespace Gallery.GalleryScenes
{
	public class GallerySceneController : ISceneController
	{
		...
		public IEnumerator PlayTimedStep(string name, float time)
		{
			...
			this.ResumeAnimation();
			yield return new LoopAnimationUntilTimeOrInput(this.Scene, this.SexAnim, this.Scene.ExpandAnimationName(name), time).Handle();
		}
		...
		public IEnumerator PlayOnceStep(string name, bool skipable = false)
		{
			...
			this.ResumeAnimation();
			yield return new PlayAnimationOnce(this.Scene, this.SexAnim, this.Scene.ExpandAnimationName(name), true).Handle();
		}
